...
const createDog = async (req, res) => {
  ...
  if (!req.body.name || !req.body.age || !req.body.breed) {
    return res.status(400).json({ error: 'name, age, and breed are required to create a new dog.' })
  }
  // Make a new dog
  const dogData = {
    name: req.body.name,
...
// A function for looking up a dog by name and aging it 1 year
const ageDog = async (req, res) => {
  try {
    const doc = await Dog.findOneAndUpdate({ name: req.body.name }, { $inc: { 'age': 1 } }, {
